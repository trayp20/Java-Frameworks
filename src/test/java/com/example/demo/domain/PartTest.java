...
class PartTest {
    ...
    void testMinimumInventory() {
        int minimumInv = 1; // Test with the expected lowest inventory level
        partIn.setInv(minimumInv);
        assertEquals(minimumInv, partIn.getInv(), "Minimum inventory should match set value for inhouse part.");
        partOut.setInv(minimumInv);
        assertEquals(minimumInv, partOut.getInv(), "Minimum inventory should match set value for outsourced part.");
    }

    @Test
    void testMaximumInventory() {
        int maximumInv = 100; // Test with an arbitrary higher inventory level
        partIn.setInv(maximumInv);
        assertEquals(maximumInv, partIn.getInv(), "Maximum inventory should match set value for inhouse part.");
        partOut.setInv(maximumInv);
        assertEquals(maximumInv, partOut.getInv(), "Maximum inventory should match set value for outsourced part.");
    }
}